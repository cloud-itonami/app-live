(ns app-live.route
  ...
  build, or a network. `app-live.worker` is the only namespace that touches
...
(def routes
  ...
  docs/adr/0001 recorded was a page that said `Routes 0` beside a route table
...
(defn- xrpc-nsid
  ...
  （`svelte/src/routes/xrpc/[...path]/+server.ts` の `event.params.path` は
  残りのパス全体である。`if (!nsid)` だけが 400）。ここで 1 セグメントに
  絞ると挙動が変わる —— NSID に `/` は現れないので上流で失敗するだけだが、
  **それは移行ではなく方針変更**であり、移行の commit に紛れ込ませるべき
  ものではない。絞るなら別の決定として記録する。"
  [path]
    ...
    (cond
      ...
      (if (= m :post)
        (if-let [nsid (xrpc-nsid p)]
          ...
          ;; 移行前の文言をそのまま持ち越す。呼び出し側が文字列で見ている
          ;; 可能性があるので、移行で勝手に変えない。
...
(defn mcp-router-url
  ...
  `+server.ts` と同じ優先順位（AGENTGATEWAY_MCP_ROUTER_URL → MCP_ROUTER_URL
  → 既定値）で、空白だけの設定は未設定として扱う。"
