...
;; (test/app_live/route_test.cljc) はソースの判断を固定するが、bundle が
...
(def sentinel
  ...
  落ちなくなる。同型の移行（app-ongakuka）で実測されたので印を使う。"
  "SENTINEL-4b7e13")
...
(def env #js {"APP_FRAMEWORK" sentinel
              "AGENTGATEWAY_MCP_ROUTER_URL" router-url})

(defn- call [h method path]
  (let [req (js/Request. (str "https://live.etzhayyim.com" path) #js {:method method})]
         ...
         (-> (js/Promise.all
             ...
             (.then
              (fn [[page health bad pre nf mna]]
                ...
                ;; env のキーは出す、値は出さない
                (check! "page shows a var key" true (str/includes? (:body page) "APP_FRAMEWORK"))
                ...
                ;; DDS の検査は **2 つに割る**。「dads-table が在る」1 本では落ちない ——
                ;; それは view が出力する markup であって、CSS が 1 バイトも入って
                ;; いないページにも現れる。実測（このページ、2026-08-18）:
                ...
                ;; nsid 無しの XRPC は 400。移行前の文言をそのまま返す。
                (check! "POST /xrpc/ status" 400 (:status bad))
                (check! "POST /xrpc/ keeps the pre-migration message" true
